...
# Convert categorical 'Stress_Level' to numerical using LabelEncoder
le = LabelEncoder()
...
# Regression: Random Forest Regressor (better than Linear Regression for complex relationships)
...
# Classification: Random Forest Classifier (better than Logistic Regression for non-linear data)
...
# Clustering: KMeans with PCA for dimensionality reduction
...
df['Cluster'] = kmeans.fit_predict(X_clust_pca)

# Evaluate clustering model
silhouette_avg = silhouette_score(X_clust_pca, df['Cluster'])
print("\nClustering Model Evaluation:")
print(f"Silhouette Score: {silhouette_avg:.4f}")

# Visualize clusters
plt.figure(figsize=(10, 6))
sns.scatterplot(x=X_clust_pca[:, 0], y=X_clust_pca[:, 1], hue=df['Cluster'], palette='viridis')
plt.title("Clustering: PCA Components")
